...
标准 OSRacer 的车体几何在出厂前已经配置好。正常启动、建图和导航时，用户应先验证 RViz 和 costmap 表现，而不是直接修改车辆几何文件。

只有在实际改动车体硬件后，例如更换传感器支架、轮径或外壳时，才需要调整几何相关配置。
...
使用新的 Ackermann 导航配置之前，确认：

- RViz 中标准机器人模型连通；
- 出厂 wheelbase 设置没有被误改；
...
| 车体擦碰障碍物 | costmap 过于激进或 TF 偏移错误 |

## 设计原则

先把 OSRacer 当作一辆车，再把它当作一个通用 Nav2 机器人。导航设置应从交付版 OSRacer 配置开始，而不是直接复制差速机器人示例。
